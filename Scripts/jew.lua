...
ScriptConfig:AddParam("teamVis","Enhanced Visibility",SGC_TYPE_TOGGLE,false,true,nil)
ScriptConfig:AddParam("shIllu","Show Illusion",SGC_TYPE_TOGGLE,false,true,nil)
...
function Tick()
  ...
  if playing and not registered then registered = true end 
  ...
  if hero then
    ...
    if spell then
      if spell.state == LuaEntityAbility.STATE_COOLDOWN then
        return drawMgr:GetTextureId("NyanUI/pips/ulti_cooldown")
      elseif spell.state == LuaEntityAbility.STATE_NOMANA then
        return drawMgr:GetTextureId("NyanUI/pips/ulti_nomana")
      ...
      elseif spell.state == LuaEntityAbility.STATE_READY then
        return drawMgr:GetTextureId("NyanUI/pips/ulti_ready")
...
function UpdateSpells(hero)
  ...
  if advancedM[hero.handle].spellCount ~= #realSpells then
    ...
    for i,v in ipairs(realSpells) do
      ...
      advancedM[hero.handle]["spell"..i].textureId = drawMgr:GetTextureId("NyanUI/spellicons/"..GetSpellIcon(v))
...
function StructureSpells(hero)
  ...
  for i,v in ipairs(realSpells) do
        ...
        advancedM[hero.handle]["spell"..i.."cdFont"] =  drawMgr:CreateFont("cdFont"..v.handle,"Arial",math.floor(tostring(math.ceil(v.cd)):len() ~= 1 and advancedM[hero.handle]["spell"..i.."lvlFont"].tall*4/tostring(math.ceil(v.cd)):len() or advancedM[hero.handle]["spell"..i.."lvlFont"].tall*2),500)
    advancedM[hero.handle]["spell"..i] = drawMgr:CreateRect(topLeft.x + 205 + (spellSize+2)*(i-1),topLeft.y,spellSize,spellSize,0x000000FF,drawMgr:GetTextureId("NyanUI/spellicons/"..GetSpellIcon(v)))
    if v.manacost > 0 then
      local mcText = tostring(v.manacost)
      local mcSize = advFont:GetTextSize(mcText) + Vector2D(1,-3)
      advancedM[hero.handle]["spell"..i.."mana"] = drawMgr:CreateRect(topLeft.x + 204 + (spellSize+2)*(i-1) + spellSize - mcSize.x,topLeft.y + spellSize - mcSize.y,mcSize.x,mcSize.y,0x5050D0D0)
...
function UpdateAdv(hero)
  ...
  for i=1,12 do
    ...
    advancedM[hero.handle]["item"..i].textureId =drawMgr:GetTextureId("NyanUI/modifier_textures/"..GetItemIcon(item))
  ...
  if bear then
    if advancedM[hero.handle]["bear"] then
      for i=1,6 do
        ...
        local itemSize = 18
        advancedM[hero.handle]["bear"..i].textureId =drawMgr:GetTextureId("NyanUI/modifier_textures/"..GetItemIcon(item))
        local chargeText = GetItemCharge(item)
        local chargeSize = advFont:GetTextSize(chargeText)
      ...
      for i=1,6 do
        ...
        local itemTopLeft = topLeft + Vector2D(215 + 19*(i),52)
        local itemSize = 18
        advancedM[hero.handle]["bear"..i] = drawMgr:CreateRect(itemTopLeft.x,itemTopLeft.y,itemSize,itemSize,0x000000FF,drawMgr:GetTextureId("NyanUI/modifier_textures/"..GetItemIcon(item)))
        local chargeText = GetItemCharge(item)
        local chargeSize = advFont:GetTextSize(chargeText)
...
function CreateAdv(hero)
  ...
  advancedM[hero.handle].portrait = drawMgr:CreateRect(topLeft.x,topLeft.y,48,64,0x000000FF,drawMgr:GetTextureId("NyanUI/heroes_vertical/"..hero.name:gsub("npc_dota_hero_","")))
  ...
  advancedM[hero.handle].dmgIcon = drawMgr:CreateRect(topLeft.x,topLeft.y+65,64,16,0x000000FF,drawMgr:GetTextureId("NyanUI/pips/DamageSword"))
  local dmgColor = GetColor(enemyData[hero.handle].dmgBonus)
  local dmgText = tostring(math.floor((enemyData[hero.handle].dmgMax + enemyData[hero.handle].dmgMin)/2 + enemyData[hero.handle].dmgBonus))
  local dmgSize = advFont:GetTextSize(dmgText)
  ...
  advancedM[hero.handle].moveIcon = drawMgr:CreateRect(topLeft.x + 35 + 21*1,topLeft.y+48,16,16,0x000000FF,drawMgr:GetTextureId("NyanUI/pips/roam_small"))
  local moveText = tostring(math.floor(enemyData[hero.handle].movespeed))
  local moveValSize = advFont:GetTextSize(moveText)
  ...
  advancedM[hero.handle].armorIcon = drawMgr:CreateRect(topLeft.x + 35 + 21*2,topLeft.y+48,16,16,0x000000FF,drawMgr:GetTextureId("NyanUI/pips/escape_small"))
  local armorColor = GetColor(enemyData[hero.handle].bonusArmor)
  local armorText = tostring(math.floor(enemyData[hero.handle].totalArmor))
  local armorSize = advFont:GetTextSize(armorText)
  ...
  advancedM[hero.handle].mgcRsIcon = drawMgr:CreateRect(topLeft.x + 35 + 21*3,topLeft.y+48,16,16,0x000000FF,drawMgr:GetTextureId("NyanUI/pips/cary_small"))
  local baseMgcRs = nil
    if hero.name == "npc_dota_hero_meepo" then
        baseMgcRs = .35
    elseif hero.name == "npc_dota_hero_visage" then
  ...
  advancedM[hero.handle].attSpIcon = drawMgr:CreateRect(topLeft.x + 35 + 21*4,topLeft.y+48,16,16,0x000000FF,drawMgr:GetTextureId("NyanUI/pips/init_small"))
  local attSpText = tostring(math.floor(enemyData[hero.handle].attackSpeed))
  local attSpSize = advFont:GetTextSize(attSpText)
  ...
  advancedM[hero.handle].strIcon = drawMgr:CreateRect(topLeft.x + 35 + 21*5,topLeft.y+48,16,16,0x000000FF,drawMgr:GetTextureId("NyanUI/pips/str_small"))
  local strText = tostring(math.floor(enemyData[hero.handle].strengthTotal))
  local strColor = GetColor(enemyData[hero.handle].strengthTotal - enemyData[hero.handle].strength)
  local strSize = advFont:GetTextSize(strText)
  ...
  advancedM[hero.handle].agiIcon = drawMgr:CreateRect(topLeft.x + 35 + 21*6,topLeft.y+48,16,16,0x000000FF,drawMgr:GetTextureId("NyanUI/pips/agi_small"))
  local agiText = tostring(math.floor(enemyData[hero.handle].agilityTotal))
  local agiColor = GetColor(enemyData[hero.handle].agilityTotal - enemyData[hero.handle].agility)
  local agiSize = advFont:GetTextSize(agiText)
  ...
  advancedM[hero.handle].intIcon = drawMgr:CreateRect(topLeft.x + 35 + 21*7,topLeft.y+48,16,16,0x000000FF,drawMgr:GetTextureId("NyanUI/pips/int_small"))
  local intText = tostring(math.floor(enemyData[hero.handle].intellectTotal))
  local intColor = GetColor(enemyData[hero.handle].intellectTotal - enemyData[hero.handle].intellect)
  local intSize = advFont:GetTextSize(intText)
  ...
  for i=1,12 do
    ...
    advancedM[hero.handle]["item"..i] = drawMgr:CreateRect(itemTopLeft.x,itemTopLeft.y,itemSize,itemSize,0x000000FF,drawMgr:GetTextureId("NyanUI/modifier_textures/"..GetItemIcon(item)))
    local chargeText = GetItemCharge(item)
    local chargeSize = advFont:GetTextSize(chargeText)
...
function GetItemIcon(item)
  if item then
    if item.name == "item_bottle" then
            ...
            elseif item:GetProperty("CDOTA_Item_EmptyBottle","m_iStoredRuneType") == 5 then
...
function UpdateHeroData(hero,pos,override)
  ...
  if missingObjs.init then
    ...
    if not missingObjs[handle.."sideIcon"] then
      missingObjs[handle.."sideIcon"] = drawMgr:CreateRect(location.ssMonitor.x,location.ssMonitor.y+location.ssMonitor.h*missingObjs.count,32,32,0x000000FF,drawMgr:GetTextureId("NyanUI/miniheroes/"..hero.name:gsub("npc_dota_hero_","")))
...
function CreateMiniMapVisual(hero)
  ...
  missingObjs[hero.handle.."mini"] = drawMgr:CreateRect(minimap.x-8,minimap.y-8,16,16,0x000000FF,drawMgr:GetTextureId("NyanUI/miniheroes/"..hero.name:gsub("npc_dota_hero_","")))
end
...
function SetMissingMessage(hero)
  if missingObjs.heroData[hero.handle] and hero.visible then
    ...
    elseif hero:FindModifier("modifier_item_invisibility_edge_windwalk") then
      missingObjs.heroData[hero.handle].missMsg = "Invis"
    elseif hero:FindModifier("modifier_oracle_false_promise") then
      missingObjs.heroData[hero.handle].missMsg = "Invis"
    elseif hero:FindModifier("modifier_item_silver_edge_windwalk") then
      missingObjs.heroData[hero.handle].missMsg = "Invis"
